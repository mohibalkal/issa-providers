...
function buildStreamUrl$1(ctx) {
  let url = `${baseUrl$1}/api/v1/${ctx.media.type}/${ctx.media.tmdbId}`;
...
async function getStreamUrl$8(ctx) {
  ...
  return {
    ...
    captions: ((_b = data.subtitles) == null ? void 0 : _b.map((sub) => ({
      url: sub.url,
      language: sub.lang
    }))) || []
  };
...
async function getStreamUrl$7(fetcher, tmdbId, type, season, episode) {
  var _a;
  let url = `https://astra.tv/api/v1/${type}/${tmdbId}`;
...
const astraScraper = makeSourcerer({
  ...
  async scrapeMovie(input) {
    const { media, fetcher } = input;
    try {
      const streams = await getStreamUrl$7(fetcher, media.tmdbId, "movie");
      return {
        embeds: [],
        stream: streams
      };
    } catch (error) {
      console.error("Error scraping astra:", error);
      return { embeds: [], stream: [] };
    }
  },
  async scrapeShow(input) {
    const { media, fetcher } = input;
    try {
      const streams = await getStreamUrl$7(fetcher, media.tmdbId, "show", media.season.number, media.episode.number);
      return {
        embeds: [],
        stream: streams
      };
...
async function getStreamUrl$6(fetcher, tmdbId, type, season, episode) {
  var _a;
...
const ee3Scraper = makeSourcerer({
  ...
  async scrapeMovie(input) {
    const { media, fetcher } = input;
    try {
      const streams = await getStreamUrl$6(fetcher, media.tmdbId, "movie");
      return {
        embeds: [],
        stream: streams
      };
  ...
  async scrapeShow(input) {
    const { media, fetcher } = input;
    try {
      const streams = await getStreamUrl$6(fetcher, media.tmdbId, "show", media.season.number, media.episode.number);
      return {
        embeds: [],
        stream: streams
      };
...
async function getStreamUrl$5(fetcher, imdbId, type, season, episode) {
  var _a;
...
const embedSuScraper = makeSourcerer({
  ...
  async scrapeMovie(input) {
    ...
    try {
      ...
      const streams = await getStreamUrl$5(fetcher, media.imdbId, "movie");
      return {
        embeds: [],
        stream: streams
      };
  ...
  async scrapeShow(input) {
    ...
    try {
      ...
      const streams = await getStreamUrl$5(fetcher, media.imdbId, "show", media.season.number, media.episode.number);
      return {
        embeds: [],
        stream: streams
      };
...
async function getStreamUrl$4(ctx) {
  ...
  const data = await ctx.proxiedFetcher(url, { headers });
  if (!((_a = data == null ? void 0 : data.streams) == null ? void 0 : _a.length)) {
    throw new NotFoundError("No streams found");
  }
  return {
...
async function getStreamUrl$2(fetcher, tmdbId, type, season, episode) {
  var _a, _b, _c;
  let url;
  if (type === "movie") {
  ...
  const captions = [];
  (_c = data.result.subtitles) == null ? void 0 : _c.forEach((sub) => {
    if (!sub.lang) return;
    captions.push({
      id: `vidsrc_${sub.lang}`,
      language: sub.lang,
...
const vidsrcScraper = makeSourcerer({
  ...
  async scrapeMovie(input) {
    const { media, fetcher } = input;
    try {
      const streams = await getStreamUrl$2(fetcher, media.tmdbId, "movie");
      return {
        embeds: [],
        stream: streams
      };
  ...
  async scrapeShow(input) {
    const { media, fetcher } = input;
    try {
      const streams = await getStreamUrl$2(fetcher, media.tmdbId, "show", media.season.number, media.episode.number);
      return {
        embeds: [],
        stream: streams
      };
...
async function searchTorrents(fetcher, query) {
...
async function getStreamUrl(fetcher, magnet) {
